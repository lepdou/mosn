...
func getServiceAwareMeta(ctx context.Context, frame *Frame) (map[string]string, error) {
	...
	case 2:
		m, err2 := decodeHessian(ctx, frame)
		if err2 != nil {
			return m, err2
		}
...
func decodeHessian(ctx context.Context, frame *Frame) (meta map[string]string, err error) {
	meta = make(map[string]string, 8)
	...
	if err != nil {
		return meta, fmt.Errorf("[xprotocol][dubbo] hessian decode framework version fail: %v", err)
	}
	frameworkVersion, ok = field.(string)
	if !ok {
		return meta, fmt.Errorf("[xprotocol][dubbo] hessian decode framework version {%v} type error", field)
	}
	meta[FrameworkVersionNameHeader] = frameworkVersion
